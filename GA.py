"""Genetic Algorithm functionality for solving TSPs
...
import ClosestEdgeInsertion as ce

LOG_BIT = True
LOG_GEN_BIT = False
TEST_LOG_BIT = False

def calcPathDistance(path, distanceList):
        distance = 0.0
        for index, node in enumerate(path):
            if (index < len(path)-1):
                distance += distanceList[node][path[index+1]]
        return distance

## Crossover functions
def crossover_onepoint(path1, path2, k = None):
    """One-point crossover function attempts.
    https://www.tutorialspoint.com/genetic_algorithms/genetic_algorithms_crossover.htm
    Returns path with randomized one-point crossover applied.
    """
    point = random.randint(1, len(path1)-1)
    path1_tail = path1[point:]
    path2_tail = path2[point:]
    ## swap tail of parents
    offspring1 = path1[:point] + path2_tail
    offspring2 = path2[:point] + path1_tail
    return (offspring1, offspring2)


def crossover_multipoint(path1, path2, numOfSplits):
    """Multi-point crossover
    https://www.tutorialspoint.com/genetic_algorithms/genetic_algorithms_crossover.htm 
    Returns path with randomized multi-point crossover applied.
    """

def crossover_pmx(path1, path2, k = None):
    """Partially Matched Crossover (PMX)
    http://www.rubicite.com/Tutorials/GeneticAlgorithms/CrossoverOperators/PMXCrossoverOperator.aspx
    Returns path with randomized PMX crossover applied.
    """
    ...
    ## check same segment of parent 2 and 
    for i, node in enumerate(path2[start:end+1]):
        path2Val = node
        if node not in offspring1:
            for _ in range(end+1 - start):    
                ## note index
                index = path2.index(path2Val)
                ## find value of corresponding value in path1 (path1[index])
                path1Val = path1[index]
                ## if node (val) is in original segment return to first step
                if path1Val not in path2[start:end+1]:
                    offspring1[path2.index(path1Val)] = node
                    break
                path2Val = path1Val

    ## fill in offspring within remaining nodes
    for i, node in enumerate(offspring1):
        if node == None:
            offspring1[i] = path2[i]
...
## Mutation functions
def mutation_swap(path, swaps = None):
    """Swaps nodes nodes in *path*.
    The nodes to be swapped are chosen at random.
    :param path: path of new chromosome to be mutated
    :param swaps: # of times to perform swap mutation on *path*, default = 1
    :returns: path with swap mutation applied.
    """
    ## restrict path to internal nodes (not first and last which are pre-determined)
    pathStart = path[0]
    pathEnd = path[-1]
    path = path[1:-1]
    ## default number of swaps
    if swaps == None:
        swaps = random.randint(1,3)
    ## random node selection
    for dummy in range(swaps):
        x = random.choices([x for x in range(0, len(path))], k=2)
        ## save value at first index for swapping
        swap = path[x[0]]
        path[x[0]] = path[x[1]]
        path[x[1]] = swap
...
def mutation_scramble(path, range = None):
    """Mutates path by selecting a range of consecutive cities/nodes in 
    *path* and scrambling the order in which they are visited.
    :param path: path of new chromosome to be mutated
    ...
    ## default range
    if range == None:
        range = random.randint( 2, math.floor(len(path)/2) )
    elif range >= len(path):
    ...
    ## shuffle range in place
    random.shuffle(path[start:end])
    path = [pathStart] + path + [pathEnd]
    return path
